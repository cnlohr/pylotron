...
static const uint8_t PlayerColors[] = { 0x06, 0x24, 0x20, 0xF0, 0xC0, 0xC6, 0x5b, 0x01 };
static const uint8_t keymap[] = { 81, 83, 82, 84, 'a', 'd', 'w', 's', 227, ' ' }; //227 = left control
...
static int last_boolet = MAX_THINGS;
static uint8_t gtickcount = 0;
static uint16_t gameover = 0;
static uint8_t  pylons_remaining = 0;
...
static int ComputeRank( int player )
{
	int rank = 0, i;
	for( i = 0; i < MAX_RFB_CONNS; i++ )
	{
		...
		//We do this to avoid bugs if the scores are the same.
		if( (FPSs[i].points * MAX_RFB_CONNS)+(i) > (FPSs[player].points * MAX_RFB_CONNS)+(player) )
		{
			rank++;
		}
	}
...
void FreeFrameForClient( int conn )
{
	...
	if( ( f->sendmode & 1 ) == 0 ) //Make sure this happens every other send.
	{
		if( !gameover )
		{
	...
	else
	{
		...
		if( gameover )
		{
			...
			ypos = ( ComputeRank( showplayer ) + 2) * 36;
		...
		case 1:	sprintf( stabuffer, "%5d \007", show->points ); break;
...
void HitPylon( int pylon, int boolet )
{
	...
	if( ph < 128 )
	{
		...
		FPSs[b->player_associated].points += 50;	
	}
	else
	{
		FPSs[b->player_associated].points ++;
	}
